...
$(function () {
    ...
    function eventTable() {
        $('#parent-bulletin-table').DataTable({
            processing: true,
            info: true,
            dom: "<'row'<'col-2 col-md-2'l><'col-4 col-md-4'B><'col-sm-6 col-md-6'f>>" +
                "<'row'<'col-sm-12'tr>>" +
    ...
    //     $('#parent-bulletin-table').DataTable().search(searchTerm).draw();
    // });
    ...
    // $('#parent-bulletin-table').on('click', '.preview-link', function (e) {
    //     e.preventDefault();
    //     const fileUrl = $(this).data('file-url');
    ...
    $('#pdfSearchInput').on('keyup', function () {
        ...
        if (activeTabId === 'tab1') {
            dataTableId = '#parent-bulletin-table';
        } else if (activeTabId === 'tab2') {
            dataTableId = '#parent-bulletin-imp-table';
        }
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.code === 200) {
                $('#parent-bulletin-table').DataTable().ajax.reload(null, false);
                $('#parent-bulletin-imp-table').DataTable().ajax.reload(null, false);
