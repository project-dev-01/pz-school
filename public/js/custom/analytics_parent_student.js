$(function () {
    ...
    function setLocalStorageForAnalytic(classObj) {
        ...
        if (get_roll_id == "4") {
         ...
         if (get_roll_id == "5") {
            // teacher
            localStorage.removeItem("parent_analytic_details");
            localStorage.setItem('parent_analytic_details', JSON.stringify(analyticClassArr));
        }
        if (get_roll_id == "6") {
            // teacher
            localStorage.removeItem("student_analytic_details");
            localStorage.setItem('student_analytic_details', JSON.stringify(analyticClassArr));
        }
        return true;
    }
...
if (get_roll_id == "5") {
if((parent_analytic_storage)) {
    if (parent_analytic_storage) {
        var parentAnalyticStorage = JSON.parse(parent_analytic_storage);
        if (parentAnalyticStorage.length == 1) {
            var classID, sectionID, subjectID, studentID, semesterID, sessionID, userBranchID, userRoleID, userID;
            parentAnalyticStorage.forEach(function (user) {
...
if (get_roll_id == "6") {
    if((student_analytic_storage)) {
        if (student_analytic_storage) {
            varstudentAnalyticStorage = JSON.parse(student_analytic_storage);
            if (varstudentAnalyticStorage.length == 1) {
                var classID, sectionID, subjectID, studentID, semesterID, sessionID, userBranchID, userRoleID, userID;
                varstudentAnalyticStorage.forEach(function (user) {
                    classID = user.class_id;
                    sectionID = user.section_id;
                    subjectID = user.subject_id;
                    semesterID = user.semester_id;
