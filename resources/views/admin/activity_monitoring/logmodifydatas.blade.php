...
@section('title',' ' . __('messages.user_modify_report') . '')
    ...
    <div class="row">
        ...
        <div class="page-title-box" style="display: inline-flex; align-items: center;margin-bottom:5px;margin-top:5px">
                ...
                <ol class="breadcrumb m-0 responsivebc">
                    ...
                    <li class="breadcrumb-item"><a href="javascript: void(0);">{{ __('messages.user_modify_report') }}</a></li>
                </ol>
                        ...
                        <div class="row">
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="role_id">{{ __('messages.user') }}<span class="text-danger">*</span></label>
                                    <select class="form-control" data-toggle="select2" id="user_id" name="user_id" data-placeholder="{{ __('messages.choose_role') }}">
                                        <option value="All">{{ __('messages.all') }}</option>
                                        @forelse($modifyusers as $users)
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="role_id">{{ __('messages.report') }}<span class="text-danger">*</span></label>
                                    <select class="form-control" data-toggle="select2" id="tablename" name="tablename" data-placeholder="{{ __('messages.choose_role') }}">
                                        <option value="All">{{ __('messages.all') }}</option>
                                        @forelse($modifytables as $table)
                                        ...
                                        <div class="table-responsive">
                                            <table class="table dt-responsive nowrap w-100" id="logactivity-table">
                                                <thead>
                                                    <tr>
                                                        <th>#</th>
                                                        <th>{{ __('messages.modify_date') }}</th>
                                                        <th>{{ __('messages.modifiedby') }}</th>
                                                        <th>{{ __('messages.modify_table') }}</th>
                                                        <th>{{ __('messages.studentstaff') }}</th>
                                                        <th>{{ __('messages.modify_datas') }}</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                </tbody>
...
<script>
    //feesGroup routes
    var log_modifylist = "{{ route('admin.log_modify.list') }}";
...
<script src="{{ asset('js/custom/logmodify.js') }}"></script>
